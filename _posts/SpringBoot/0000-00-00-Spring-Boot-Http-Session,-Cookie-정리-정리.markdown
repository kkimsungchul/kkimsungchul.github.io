...
## new Session 을 하지 않고 HttpServletRequest.getSession() 을 하는 이유  
	...
	따라서, Spring에서 세션을 사용할 때 new Session()을 사용하지 않고 HttpServletRequest.getSession()을 사용하는 이유는 세션 관리, 세션 스코프 빈 사용, 테스트 용이성 등의 이점이 있기 때문입니다.  
